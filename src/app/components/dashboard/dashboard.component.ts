...
export class DashboardComponent {
  ...
  displayedColumns: string[] = ['id', 'type', 'title', 'image', 'relationships'];
  ...
  onSort() {
    if (this.counter === 0) {
      this.counter++;
      this.dataSource = _.cloneDeep(arr).sort((a, b) => {
        const nameA = a.attributes.content;
        const nameB = b.attributes.content;
  ...
  onSearch(data: any) {
    if (data !== '') {
      this.dataSource = arr.filter(element => element.attributes.content.toLowerCase().includes(data.toLowerCase()));
    }
    if (data === '') this.dataSource = arr;
  }
...
const [arr, links] = [obj.data, obj.links];

const ELEMENT_DATA: PeriodicElement[] = arr;
